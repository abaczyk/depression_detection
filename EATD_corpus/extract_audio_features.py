...
def calculate_formant_freq_bandwidths(mfcc_val):
    ...
    # Estimate the formants using the LPC method
...
for index in range(105):
    extract_features(index + 1, audio_mel_features, audio_features, audio_file_name)

#print("Saving npz file locally...")
#np.savez(os.path.join(prefix, 'AudioFeatures/whole_samples_reg_%d.npz' % (cluster_size * 16)), audio_features)
#np.savez(os.path.join(prefix, 'AudioFeatures/whole_labels_reg_%d.npz') % (cluster_size * 16), audio_targets)
